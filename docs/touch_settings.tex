...
In this mode, the main goal is to come up with a number called a \textit{touch
...
It is the \mTe{f} mode when the \cRs{f} is in the \dLe{f} position.
...
initially mentioned, the main goal of \mTS{f} is to come up with a \textit{touch
...
\fontTGA{f}{NOT TOUCHED} case and \textit{would} have been registered in the
\fontTGA{f}{TOUCHED} case, as expected.

\par\medskip

However, if the threshold value was set to \num{15} or less, a touch event
\textit{would} have been registered in the \fontTGA{f}{NOT TOUCHED} case as well
as the \fontTGA{f}{TOUCHED} case.  And if the threshold value was greater than
...
\begin{longtabu} { X[1,c,m] | X[3,l,m] }
  ...
  \sTSRT{sym} & Set initial touch threshold by touching and reading. \\ \mrule
  \multicolumn{2}{c}{\mTSCo{sym}} \\ \mrule
  \sTSNS{sym} & Configure the base number of scans per reading. \\ \drule{2}
  \sTSPS{sym} & Configure the prescaler - the multiplier applied to \sTSNS{f}. \\ \drule{2}

  \pagebreak \drule{2}

  \sTSRC{sym} & Configure the reference charge rate per reading. \\ \drule{2}
  \sTSEC{sym} & Configure the external\slash electrode charge rate per reading. \\ \mrule
  \multicolumn{2}{c}{\mTSCa{sym} \mTSRe{sym} \mTSCo{sym}} \\ \mrule
  \sTSTA{sym} & Test and adjust final touch threshold value. \\ \mrule
...
and the \cLi{f} window will be lit \cRe{}.
...
is happening.  Progress can be seen by the \cLi{f} window cycling through the
...
can be seen by the \cLi{f} window cycling through the colors of the rainbow
...
else fails, you can \hyperref[Touch Settings - Disabled]{\mTSDi{f}} touch
altogether.
...
\textit{with} and \textit{without} \mAu{f} playing.  When the \cAm{f} is on, it
...
To reiterate, the \cBe{f} should \textit{continuously} sound when touched with
...
% Disabled
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Disabled} \sTSDi{syml} \label{Touch Settings - Disabled}
