...
\begin{tabu} { X[1,c,m] | X[3,l,m] }
  ...
  \sClSh{sym} & Display the current track. \\ \drule{2}
...
Touch can also be used to show the current track.  From \sClTi{f}, \sClMD{f}
or \sClYe{f}, you will \aTo{f} the \cTo{f} of the enclosure for \num{1} second
...
To display the current track from \sClYe{f}, simply \aPR{f} the \cEs{f}.
...
Touch can also be used to display the current track from \sClTi{f}, \sClMD{f}
and \sClYe{f}.  To do so, \aTo{f} the \cTo{f} of the enclosure for
\num{1} second.
...
most of your fingers lay across the top of the enclosure - it won't likely
be enough to just touch or tap the top with one finger.}

If you don't \aPR{f} or \aTu{f} the \cEs{f} within \num{3} seconds after
showing the track or aren't touching the top, the time will automatically
redisplay.  If you \aPR{f} before a \aTu{f},
it will immediately go back to \sClTi{f}.
...
enabled a touch time via \hyperref[Power Settings]{\mPS{ss}}, the screens
...
to \sClTi{f}. As long as you are turning the \cEs{f} or touching the top of the
...
\as{{c c c}}{%
\eOn{sym}{NIGHT LIGHT} & \sTu & \eAc{sym}{CYCLE COLORS} \\}
...
There is a \mono{CR2032} coin cell battery, apart from the rechargeable
battery, attached to the \dTo{f} that continues to keep time even when the
device if turned \sOff{f} via the \cPo{f} switch.  If the time is incorrect
when powering the device \sOn{f}, the battery may need replacement.  For more
...
In \sClTi{f}, \sClMD{f} and \sClYe{f}, the screens will \sDim{f} when the device
is in a \sPoNa{f} power state.

\par\medskip

In \sClTi{f}, \sClMD{f}, \sClYe{f} and \sClSh{f}, the screens will turn \sOff{f}
when the device is in a \sPoSl{f} power state.
...
\begin{table}[H]
  ...
  \multirow{2}{*}[-1mm]{\sPoNa{sym}} 
    & \sClTi{sym} \sClMD{sym} \sClYe{sym} & \sDim{sym} \\ \dcrule{2}{3}
  & \sClSh{sym} \sClSe{sym} & \sOn{sym} \\ \mrule
