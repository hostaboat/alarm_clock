...
On the \cBa{f} of the device is the \cPP{f} that is used for adding/removing
songs to/from the device and to both power the device and charge the
\hyperref[Rechargeable Battery]{\cRB{f}} inside.
...
The \cPP{f} is a \cUSB{f} port that allows for both data transfer and power
to the device.
...
\begin{itemize}
  \item When connected to a computer, make sure the device is ``ejected'' before
    unplugging the \hyperref[USB Cable]{\cUsb{f}}.
  \item The device can be \sON{f} or \sOFF{f} when plugging or unplugging the
    \hyperref[Power Adapter]{\cPA{f}} or \hyperref[USB Cable]{\cUsb{f}}.
...
See \hyperref[Powering and Recharging]{Powering \& Recharging} and the
\hyperref[USB]{USB} sections for more information.

\danger{Always eject the device before unplugging it from a computer to
avoid possible filesystem corruption.}
...
\hyperref[USB Port]{\cPP{f}}.  It can accept either \num{110} or \num{240}
...
The \cUsb{f} is a \textit{USB 2.0 Standard Type-A Male to Micro Type-B Male}.
...
rate will be significantly slower compared to using the \cPA{f}.
...
\info{The cable must be a USB 2.0 cable and \textbf{not} a USB 3.0 cable and
must be a \textit{USB 2.0 Standard Type-A Male to Micro Type-B Male}.}

\info{There is also a USB 2.0 Mini connector that is similar in appearance to
the USB 2.0 Micro.  A USB Mini connector will \textbf{not} fit in the \cPP{f}.}
...
The figure below indicates how the \cUSB{f} plug should be oriented when
\textit{facing} the \hyperref[USB Port]{\cPP{f}}.
...
Using the \hyperref[Power Adapter]{\cPA{f}}, plug the \cUSB{f} end into the
\cPP{f} on the \cBa{f} of the device and the 2-prong end into a wall or power
strip receptacle.

\par\medskip

Using the \hyperref[USB Cable]{\cUsb{f}}, connect the Micro-B end into the
\cPP{f} on the device and the Standard Type-A end into a USB port on a computer
or USB hub.
...
Using the \hyperref[Power Adapter]{\cPA{f}}, it should take about \num{8} hours
to charge if the \hyperref[Low Battery Indicator]{\cLB{f}} is signaling - plug
it in before going to bed and it should be charged by the time you wake up.

\par\medskip

Using the \hyperref[USB Cable]{\cUsb{f}}, charging will likely take longer since
USB ports on a computer generally do not supply as much current as the
\hyperref[Power Adapter]{\cPA{f}} will.
